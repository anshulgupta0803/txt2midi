...
class UiApplication(tk.Frame):
	...
	def __init__(self, master=None):
		...
		self.font_courier18 = font.Font(family='Arial',size=18);
	...
	def createPanelST(self):
		...
		panel.lbl_src.place(x=80, y=y_offset)
		...
		panel.opt_inst_txtvar.set("Keyboard") # default value
		panel.opt_inst= tk.OptionMenu(panel, panel.opt_inst_txtvar, "Keyboard", "Guitar", "Vilon")
		...
		panel.opt_inst.place(x=f_x,y=y_offset+((f_y_inc*row)),height=f_height, width=f_width-160);	
	...
	def createPanelStatus(self):
		...
		panel.btn_play["text"] = "Play";
		panel.btn_play["command"]=self.onClickPlay;
		panel.btn_play.place(x=20,y=y_offset+((f_y_inc*row)),height=30, width=110)
